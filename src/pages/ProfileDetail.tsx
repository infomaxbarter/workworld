...
interface Profile {
  ...
  lat: number | null; lng: number | null; approved: boolean; slug: string | null; created_at: string;
}
...
const ProfileDetail = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) { const p = data as unknown as Profile; setProfile(p); setForm(p); }
          ...
          {editing ? (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1 sm:col-span-2"><Label>Instagram</Label><Input value={form.instagram || ''} onChange={(e) => setForm({ ...form, instagram: e.target.value })} placeholder="@username" /></div>
              </div>
              <div className="space-y-2">
